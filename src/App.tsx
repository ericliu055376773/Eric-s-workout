...
const isFirebaseConfigured =
  firebaseConfig.apiKey && !firebaseConfig.apiKey.includes('請填寫');

const app = isFirebaseConfigured ? initializeApp(firebaseConfig) : null;
const auth = isFirebaseConfigured ? getAuth(app) : null;
const db = isFirebaseConfigured ? getFirestore(app) : null;
const appId =
  typeof __app_id !== 'undefined' ? __app_id : 'fitness-tracker-app-v3';
...
export default function FitnessApp() {
  ...
  function changeDate(days) {
    ...
    setSelectedDate(
      new Date(current.getTime() - offset).toISOString().split('T')[0]
    );
  ...
  const stats = useMemo(() => {
    ...
    const weekStartStr = new Date(weekStart.getTime() - offset)
      .toISOString()
      .split('T')[0];
    const weekEndStr = new Date(weekEnd.getTime() - offset)
      .toISOString()
      .split('T')[0];
    ...
    if (savedTodayLog && savedTodayLog.exercises) {
      savedTodayLog.exercises.forEach((ex) => {
        if (
          savedTodayLog.trainingParts &&
          savedTodayLog.trainingParts.includes(ex.part)
        ) {
  ...
  const handleSaveLog = async () => {
    ...
    setSaving(true);
    try {
      const filteredExercises = dailyExercises.filter((ex) =>
        trainingParts.includes(ex.part)
      );
      const logDocRef = doc(
        db,
        'artifacts',
  ...
  const handleAddSupplement = () => {
    ...
    const currentSuppsInPeriod =
      settings.supplements[settingActivePeriod] || [];
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-zinc-950 text-zinc-100 p-6">
        <div className="bg-red-900/20 border border-red-500/50 rounded-2xl p-6 max-w-md w-full shadow-lg">
          ...
          <div className="space-y-4 text-sm text-zinc-300 leading-relaxed">
            <p>
              目前的「白畫面」是因為 React 缺少資料庫連線資訊，導致執行崩潰。
            </p>
            <p>
              請在左側程式碼大約 <strong>第 35 行</strong> 的{' '}
              <code className="bg-zinc-800 text-emerald-400 px-1.5 py-0.5 rounded">
                firebaseConfig
              </code>{' '}
              變數中，填入您從 Firebase 主控台取得的專案金鑰。
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="max-w-md mx-auto px-4 pb-4 space-y-3">
            ...
            <div className="flex items-center justify-between bg-zinc-950 rounded-xl p-2 border border-zinc-800">
              ...
              <div className="flex items-center gap-2 font-medium">
                ...
                <span className="tracking-wide">
                  {selectedDate === getTodayString() ? '今天 ' : ''}
                  {selectedDate}
                </span>
              </div>
              <button
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-zinc-800/40 border border-zinc-700/50 rounded-xl p-3 flex flex-col items-center justify-center">
                <span className="text-[10px] text-zinc-400 mb-1 flex items-center gap-1">
                  <Activity size={12} />
                  今日
                </span>
                <span className="font-bold text-zinc-100 text-lg leading-none">
                  {totalDailyWeight.toLocaleString()}
                </span>
              </div>
              <div className="bg-emerald-900/20 border border-emerald-800/30 rounded-xl p-3 flex flex-col items-center justify-center">
                <span className="text-[10px] text-emerald-500 mb-1 flex items-center gap-1">
                  <Target size={12} />
                  本週
                </span>
                <span className="font-bold text-emerald-400 text-lg leading-none">
              ...
              <div className="bg-blue-900/20 border border-blue-800/30 rounded-xl p-3 flex flex-col items-center justify-center">
                <span className="text-[10px] text-blue-500 mb-1 flex items-center gap-1">
                  <Target size={12} />
                  本月
                </span>
                <span className="font-bold text-blue-400 text-lg leading-none">
        ...
        {view === 'main' ? (
          ...
          <>
            ...
            {trainingParts.length > 0 && (
              <section className="space-y-4">
                {trainingParts.map((part) => {
                  ...
                  const exercisesForPart = dailyExercises.filter(
                    (ex) => ex.part === part
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      {exercisesForPart.length === 0 ? (
                        <p className="text-zinc-500 text-sm text-center py-4 bg-zinc-950/30 rounded-xl border border-dashed border-zinc-800">
                          尚未新增動作
                        </p>
                      ) : (
                        <div className="space-y-3">
                          ...
                          {exercisesForPart.map((ex) => (
                            <div
                              key={ex.id}
                              className="grid grid-cols-[1fr_4rem_4rem_2rem] gap-2 items-center bg-zinc-950/50 p-2 rounded-xl border border-zinc-800/50"
                            >
                              <select
                                value={ex.name}
                                onChange={(e) =>
            ...
            <section className="bg-zinc-900 rounded-2xl p-5 border border-zinc-800">
              ...
              Object.values(settings.supplements).every(
                (arr) => arr.length === 0
              ) ? (
                <p className="text-zinc-500 text-sm text-center py-4 bg-zinc-950/30 rounded-xl border border-dashed border-zinc-800">
                  尚未設定補給品，請至後台新增。
                ...
                <div className="space-y-4">
                  {Object.entries(settings.supplements).map(
                    ([period, supps]) => {
                      if (!supps || supps.length === 0) return null;
                      return (
                        <div
                          key={period}
                          className="bg-zinc-950/50 rounded-xl p-3 border border-zinc-800/50"
                        >
          ...
          <div className="space-y-6">
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {settings.parts.map((part) => (
                    <div
                      key={part}
                      className="flex items-center gap-1.5 bg-zinc-800 border border-zinc-700 rounded-lg pl-3 pr-1 py-1 shadow-sm"
                    >
                      ...
                      <button
                        ...
                        className="p-1 text-zinc-500 hover:text-red-400 hover:bg-zinc-700 rounded-md transition-colors"
                      >
                        <X size={14} />
                      </button>
                    </div>
                  ...
                  <div className="bg-zinc-950/50 rounded-xl p-4 border border-zinc-800/80 space-y-4">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onKeyDown={(e) =>
                          e.key === 'Enter' && handleAddExerciseToSettings()
                        }
                      ...
                      />
                      <button
                        onClick={handleAddExerciseToSettings}
                    ...
                    <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
                      {(settings.exercises[settingActivePart] || []).length ===
                        0 && (
                        <p className="text-zinc-600 text-sm py-2">
                          目前沒有動作選項。
                        </p>
                      )}
                      {(settings.exercises[settingActivePart] || []).map(
                        (ex) => (
                          <div
                            key={ex}
                          ...
                          >
                            ...
                            <button
                              onClick={() =>
                                handleRemoveExerciseFromSettings(
                                  settingActivePart,
                                  ex
                                )
                              }
                              className="text-zinc-500 hover:text-red-400 p-1.5 hover:bg-zinc-800 rounded-md transition-colors"
                            >
                              <Trash2 size={16} />
                            </button>
                          </div>
            ...
            <section className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
              <div className="bg-zinc-800/40 p-4 border-b border-zinc-800 flex items-center gap-2">
                ...
                <h2 className="font-bold">3. 分段補給品設定</h2>
              </div>
              <div className="p-4 space-y-4">
                <div className="flex gap-2">
                  <select
                  ...
                  <input
                    ...
                    onKeyDown={(e) =>
                      e.key === 'Enter' && handleAddSupplement()
                    }
                    placeholder="新增補品名稱..."
                  ...
                  <button
                    ...
                    className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors shrink-0"
                  >
                    新增
                  </button>
                </div>
                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-2">
                          {supps.map((supp) => (
                            <div
                              key={supp}
                              className="flex items-center gap-1 bg-zinc-800 border border-zinc-700 rounded-lg pl-3 pr-1 py-1"
                            >
                              ...
                              <button
                                onClick={() =>
                                  handleRemoveSupplement(period, supp)
                                }
                                className="p-1 text-zinc-500 hover:text-red-400 hover:bg-zinc-700 rounded-md transition-colors"
                              >
                                <X size={14} />
          ...
          <div className="max-w-md mx-auto pointer-events-auto">
            <button
              ...
              className={`w-full py-4 rounded-2xl font-black text-lg flex items-center justify-center gap-2 transition-all duration-300 shadow-xl ${
                saveSuccess
                  ? 'bg-emerald-400 text-zinc-950 shadow-[0_0_30px_rgba(52,211,153,0.5)] scale-105'
