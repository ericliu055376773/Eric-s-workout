...
export default function FitnessApp() {
  ...
  const activeCyclesInfo = useMemo(() => {
    ...
    return settings.cycles.map(cycle => {
      const start = new Date(cycle.startDate);
      const target = new Date(selectedDate);
      const diffDays = Math.floor((target.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));

      if (diffDays < 0) {
        return { ...cycle, status: '尚未開始', currentPhase: null };
      }

      let daysAccumulated = 0;
      for (let i = 0; i < cycle.phases.length; i++) {
        const phase = cycle.phases[i];
        if (diffDays < daysAccumulated + phase.days) {
          return {
            ...cycle,
            status: '進行中',
            currentPhase: phase,
            dayInPhase: diffDays - daysAccumulated + 1,
            progressPercent: Math.round(((diffDays - daysAccumulated + 1) / phase.days) * 100)
          };
        }
        daysAccumulated += phase.days;
      }
      
      return { ...cycle, status: '已結束', currentPhase: null };
    });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(settingsRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (Array.isArray(data.supplements)) data.supplements = { '未分類': data.supplements };
        ...
        setSettings(data);
        if (!data.parts.includes(settingActivePart)) setSettingActivePart(data.parts[0] || '');
        ...
        if (!data.parts.includes(analysisPart)) setAnalysisPart(data.parts[0] || '');
      } else {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { handleSaveLog(true); }, 1500);
  ...
  const updateExercise = (id, field, value) => {
    setDailyExercises(prev => prev.map(ex => ex.id === id ? { ...ex, [field]: value } : ex ));
    setIsDirty(true);
  };
  ...
  const handleAddPart = () => {
    ...
    saveSettingsToDB({ ...settings, parts: [...settings.parts, newPart.trim()] });
    setNewPart('');
  };

  const handleRemovePart = (part) => {
    const updatedParts = settings.parts.filter(p => p !== part);
    saveSettingsToDB({ ...settings, parts: updatedParts });
    if (settingActivePart === part) setSettingActivePart(updatedParts[0] || '');
  };
  ...
  const handleRemovePeriod = (period) => {
    ...
    saveSettingsToDB({ ...settings, supplementPeriods: updatedPeriods, supplements: updatedSupplements });
    if (settingActivePeriod === period) setSettingActivePeriod(updatedPeriods[0] || '');
  };
  ...
  const handleDrop = (e, targetPeriod) => {
    e.preventDefault();
    if (!draggedPeriod || draggedPeriod === targetPeriod) return;
    
    const periods = [...(settings.supplementPeriods || [])];
    const draggedIdx = periods.indexOf(draggedPeriod);
    const targetIdx = periods.indexOf(targetPeriod);
    
    periods.splice(draggedIdx, 1);
    periods.splice(targetIdx, 0, draggedPeriod);
    
    saveSettingsToDB({ ...settings, supplementPeriods: periods });
  ...
  const addDefaultCreatineCycle = () => {
    const newCycle = {
      ...
      phases: [
        { name: '填充期', days: 7, dosage: '28克 / 分4次' },
        { name: '維持期', days: 45, dosage: '14克 / 天' },
        { name: '休息期', days: 30, dosage: '暫停使用' }
      ]
    };
    saveSettingsToDB({ ...settings, cycles: [...(settings.cycles || []), newCycle] });
  };
          ...
          <div className="max-w-md mx-auto px-4 pb-4">
            <div className="flex items-center justify-between bg-zinc-950 rounded-xl p-2 border border-zinc-800">
              ...
              <div className="flex items-center gap-2 font-medium">
                ...
                <span className="tracking-wide">{selectedDate === getTodayString() ? '今天 ' : ''}{selectedDate}</span>
              </div>
              <button onClick={() => changeDate(1)} className="p-2 hover:bg-zinc-800 rounded-md transition-colors disabled:opacity-30 text-zinc-400" disabled={selectedDate === getTodayString()}>
          ...
          <>
            ...
            {activeCyclesInfo.length > 0 && (
              <section className="space-y-3">
                {activeCyclesInfo.map(cycle => (
                  <div key={cycle.id} className="bg-zinc-900 border border-blue-900/50 rounded-2xl p-4 shadow-sm relative overflow-hidden">
                    <div className="absolute top-0 left-0 h-1 bg-zinc-800 w-full">
                      <div className="h-full bg-blue-500 transition-all duration-500" style={{ width: `${cycle.progressPercent || 0}%` }}></div>
                    </div>
                        ...
                        {cycle.status === '進行中' ? (
                          <div className="mt-2 space-y-1">
                            <div className="text-sm text-zinc-200">
                              當前階段: <span className="font-bold text-white">{cycle.currentPhase.name}</span> 
                              <span className="text-zinc-500 text-xs ml-2">(第 {cycle.dayInPhase} 天 / 共 {cycle.currentPhase.days} 天)</span>
                            </div>
                            <div className="text-xs font-bold text-blue-400 bg-blue-500/10 inline-block px-2 py-1 rounded-md mt-1">
                              今日建議用量: {cycle.currentPhase.dosage}
                            </div>
                          </div>
                        ) : (
                          <div className="mt-2 text-sm text-zinc-500">{cycle.status}</div>
                        )}
                      </div>
                    </div>
                  </div>
                            ...
                            return (
                              <div key={ex.id} className="bg-zinc-950/50 p-2.5 rounded-xl border border-zinc-800/50 space-y-3">
                                ...
                                <div className="bg-zinc-900/80 rounded-lg p-2 text-[11px] flex justify-between items-center border border-zinc-800/50">
                                  ...
                                  {progressStatus === 'up' && (<div className="flex items-center gap-1 text-yellow-400 font-bold bg-yellow-400/10 px-2 py-0.5 rounded-full"><TrendingUp size={12} /> 進步 {diff}kg</div>)}
                                  {progressStatus === 'down' && (<div className="flex items-center gap-1 text-red-400 font-bold bg-red-400/10 px-2 py-0.5 rounded-full"><TrendingDown size={12} /> 退步 {Math.abs(diff)}kg</div>)}
                                  {progressStatus === 'same' && (<div className="flex items-center gap-1 text-zinc-500 font-bold bg-zinc-800 px-2 py-0.5 rounded-full"><Minus size={12} /> 持平</div>)}
                                </div>
                              </div>
                            );
                          })}
            ...
            {/* 補給品清單 (依排序後的 supplementPeriods 顯示) */}
            <section className="bg-zinc-900 rounded-2xl p-5 border border-zinc-800">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm text-zinc-400 font-medium flex items-center gap-2"><Pill size={16} /> 今日補給清單</h2>
              </div>
              {Object.keys(settings.supplements || {}).length === 0 || Object.values(settings.supplements).every(arr => arr.length === 0) ? (
                  ...
                  {(settings.supplementPeriods || []).map(period => {
                    ...
                    return (
                      <div key={period} className="bg-zinc-950/50 rounded-xl p-3 border border-zinc-800/50">
                        <div className="text-xs font-bold text-zinc-500 mb-2 flex items-center gap-1"><Clock size={12} /> {period}</div>
            ...
            <section className="bg-zinc-900 rounded-2xl p-5 border border-zinc-800 flex items-center justify-between shadow-sm">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-pink-500/10 rounded-lg"><Weight className="text-pink-400" size={20} /></div>
                <div>
                  <h2 className="text-sm text-zinc-300 font-bold">今日體重</h2>
                  {settings.targetWeight && (<p className="text-[10px] text-zinc-500 mt-0.5">目標: {settings.targetWeight} kg</p>)}
                </div>
              </div>
            ...
            <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 shadow-sm">
              ...
              <div className="mb-6">
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                  {settings.parts.map(part => {
                    if (part === '休息') return null;
                    return (
                      <button key={part} onClick={() => setAnalysisPart(part)} className={`whitespace-nowrap px-4 py-2 rounded-xl text-sm font-bold transition-all ${analysisPart === part ? 'bg-emerald-500 text-zinc-950 shadow-md' : 'bg-zinc-950 border border-zinc-800 text-zinc-400 hover:border-zinc-600'}`}>{part}</button>
                    );
                  })}
                </div>
              </div>
              <div className="bg-zinc-950/50 rounded-xl border border-zinc-800 p-4 space-y-4">
                ...
                <div className="grid grid-cols-[1fr_auto_1fr] gap-4 items-center">
                  ...
                  <div className="text-zinc-600"><ArrowLeft size={20} className="rotate-180" /></div>
                ...
                <div className="pt-2 flex justify-center">
                   {partAnalytics.current.date === '--' && partAnalytics.previous.date === '--' ? (
                     ...
                     <div className="flex items-center gap-1 text-emerald-400 bg-emerald-400/10 px-3 py-1 rounded-full text-sm font-medium"><TrendingUp size={16} /> 進步了 {partAnalytics.diff.toLocaleString()} kg!</div>
                   ) : partAnalytics.isDecrease ? (
                     <div className="flex items-center gap-1 text-red-400 bg-red-400/10 px-3 py-1 rounded-full text-sm font-medium"><TrendingDown size={16} /> 退步了 {partAnalytics.diff.toLocaleString()} kg</div>
                   ) : (
                     <div className="flex items-center gap-1 text-zinc-400 bg-zinc-800 px-3 py-1 rounded-full text-sm font-medium"><Minus size={16} /> 持平</div>
                   )}
                </div>
              </div>
            </section>
            <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 shadow-sm">
              <h3 className="font-bold text-zinc-100 mb-4 flex items-center gap-2"><History size={18} className="text-zinc-400" /> 各部位近期重量紀錄</h3>
                ...
                {settings.parts.map(part => {
                  ...
                  return (
                    <div key={part} className="bg-zinc-950 border border-zinc-800/80 rounded-xl p-3 flex flex-col justify-center">
                      <span className="text-xs text-zinc-400 mb-1 flex items-center justify-between">{part}<span className="text-[10px] text-zinc-600">{record ? record.date.substring(5) : '--'}</span></span>
                      <span className="font-bold text-zinc-200 text-xl leading-none">{record ? record.volume.toLocaleString() : 0} <span className="text-xs font-medium text-zinc-500">kg</span></span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 shadow-sm">
              ...
              {weightAnalytics.data.length === 0 ? (
                <div className="text-center py-12 text-zinc-500 text-sm bg-zinc-950/50 rounded-xl border border-dashed border-zinc-800"><Weight className="mx-auto mb-2 opacity-50" size={32} />目前還沒有任何體重紀錄喔！<br/>回到首頁輸入體重開始追蹤。</div>
              ) : (
                <>
                  <div className="grid grid-cols-2 gap-3 mb-6">
                    <div className="bg-zinc-950/50 p-3 rounded-xl border border-zinc-800"><div className="text-xs text-zinc-500 mb-1">最新體重</div><div className="text-xl font-bold text-pink-400">{weightAnalytics.latest} kg</div></div>
                    <div className="bg-zinc-950/50 p-3 rounded-xl border border-zinc-800">
                      ...
                      <div className="text-xl font-bold text-zinc-200">
                        {settings.targetWeight ? ((weightAnalytics.latest - settings.targetWeight) > 0 ? `+${(weightAnalytics.latest - settings.targetWeight).toFixed(1)} kg` : `${(weightAnalytics.latest - settings.targetWeight).toFixed(1)} kg`) : '--'}
                      </div>
                    </div>
                    <div className="bg-zinc-950/50 p-3 rounded-xl border border-zinc-800"><div className="text-xs text-zinc-500 mb-1">歷史最高</div><div className="text-lg font-bold text-zinc-300">{weightAnalytics.highest} kg</div></div>
                    <div className="bg-zinc-950/50 p-3 rounded-xl border border-zinc-800"><div className="text-xs text-zinc-500 mb-1">歷史最低</div><div className="text-lg font-bold text-zinc-300">{weightAnalytics.lowest} kg</div></div>
                  </div>
                    ...
                    <div className="h-64 w-full bg-zinc-950/80 p-4 rounded-xl border border-zinc-800 pt-8">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={weightAnalytics.data}>
                          ...
                          <Tooltip contentStyle={{ backgroundColor: '#18181b', border: '1px solid #27272a', borderRadius: '8px' }} itemStyle={{ color: '#f472b6' }}/>
                          {settings.targetWeight && (<ReferenceLine y={settings.targetWeight} stroke="#10b981" strokeDasharray="3 3" label={{ position: 'top', value: '目標', fill: '#10b981', fontSize: 10 }} />)}
                          <Line type="monotone" dataKey="weight" stroke="#f472b6" strokeWidth={3} dot={{ fill: '#f472b6', strokeWidth: 2, r: 4 }} activeDot={{ r: 6 }} />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                  ) : (
                    <div className="text-center py-6 text-zinc-500 text-xs bg-zinc-950/50 rounded-xl border border-zinc-800">需要至少兩天的體重紀錄才能繪製趨勢線喔！</div>
                  )}
                </>
              )}
            </section>
          ...
          <div className="space-y-6">
            ...
            <section className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="p-4 space-y-4">
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                  {settings.parts.map(part => (
                    <button key={part} onClick={() => setSettingActivePart(part)} className={`whitespace-nowrap px-4 py-2 rounded-xl text-sm font-bold transition-all ${settingActivePart === part ? 'bg-emerald-500 text-zinc-950 shadow-md' : 'bg-zinc-950 border border-zinc-800 text-zinc-400 hover:border-zinc-600'}`}>{part}</button>
                  ))}
                </div>
                {settingActivePart ? (
            ...
            <section className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="p-4 space-y-4">
                <div className="flex items-center gap-4 bg-zinc-950/50 rounded-xl p-3 border border-zinc-800/50">
                  ...
                  <div className="flex items-center gap-2">
                    <input type="number" min="1" value={settings.restTimer || 60} onChange={(e) => { const val = Number(e.target.value); if (val > 0) saveSettingsToDB({ ...settings, restTimer: val }); }} className="w-20 bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-center text-sm font-mono text-yellow-400 focus:outline-none focus:border-yellow-500 shadow-inner" />
            ...
            <section className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="p-4 space-y-4">
                <div className="flex items-center gap-4 bg-zinc-950/50 rounded-xl p-3 border border-zinc-800/50">
                  ...
                  <div className="flex items-center gap-2">
                    <input type="number" step="0.1" min="0" value={settings.targetWeight || ''} onChange={(e) => { const val = e.target.value; saveSettingsToDB({ ...settings, targetWeight: val ? Number(val) : '' }); }} className="w-20 bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-center text-sm font-mono text-pink-400 focus:outline-none focus:border-pink-500 shadow-inner" />
                  ...
                  <div key={cycle.id} className="bg-zinc-950/50 rounded-xl p-4 border border-zinc-800/50 space-y-3">
                    ...
                    <div className="space-y-1 mt-2">
                      {cycle.phases.map((p, i) => (
                        <div key={i} className="text-xs text-zinc-500 flex justify-between bg-zinc-900 px-2 py-1.5 rounded">
                          <span>{p.name} ({p.days}天)</span><span className="text-zinc-300 font-medium">{p.dosage}</span>
                        </div>
                      ))}
                    </div>
                  </div>
